...
def delete_stopwords(lines):
    fp = open('去掉无用词汇之后数据.txt', 'w', encoding='utf-8')
    stopwords = read_file(stopword_file)
    all_words = []

    for line in lines:
        tmp = [word for word in jieba.cut(line) if word not in stopwords]
        fp.write(' '.join(tmp) + '\n')
        all_words += tmp
...
if __name__ == "__main__":
    ...
    #将排序结果保存到json文件中
    with open(outputword_file, "w", encoding='utf-8') as output_file:
